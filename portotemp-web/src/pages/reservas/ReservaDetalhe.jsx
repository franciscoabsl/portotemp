...
const ReservaDetalhe = ({ reserva: r, onClose }) => {
  ...
  const gerarLink = async () => {
    setLoadingLink(true);
    try {
      const result = await api.gerarLinkWhatsApp(r.id, tipoMsg, 'PT');
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <span
          className={`badge text-sm px-3 py-1 ${statusColor[status?.color]}`}
        >
          {status?.label}
        </span>
        <span className="badge bg-slate-100 text-slate-600">
          {ORIGEM_RESERVA[r.origem]}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-slate-50 rounded-xl p-4 space-y-2">
          ...
          <p className="font-bold text-slate-900">
            {formatDate(r.dataCheckin)}
          </p>
          <p className="text-sm text-slate-400">
            até {formatDate(r.dataCheckout)}
          </p>
        ...
        <div className="bg-slate-50 rounded-xl p-4 space-y-2 col-span-2">
          ...
          <div className="flex gap-6">
            ...
            <div>
              ...
              <p className="font-bold text-slate-900">
                {formatCurrency(r.taxaLimpezaHospede)}
              </p>
            </div>
          </div>
        </div>
          ...
          {linkResult && (
            <div className="bg-success-50 border border-success-100 rounded-xl px-4 py-3 space-y-2">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="btn-primary inline-flex text-xs px-3 py-1.5"
              >
